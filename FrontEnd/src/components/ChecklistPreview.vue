  ...
  <div class="checklist-preview">
    ...
    <div class="preview-actions">
      <p class="preview-header">
        {{ isEditing ? `Add new ${termsLower.checklist} for this ${termsLower.person}:` : `${termsLower.checklists} that will apply to this ${termsLower.person}:` }}
      </p>
      <button
        type="button"
        class="add-item-btn"
    ...
    <div v-if="!isEditing && filteredItems.length === 0 && pendingNewItems.length === 0" class="empty-state">
      <p>No {{ termsLower.checklists }} will apply based on the current {{ termsLower.checkpoint }} assignments.</p>
      <p v-if="pendingLocationIds.length === 0" class="help-text">
        Assign {{ termsLower.checkpoints }} to see relevant {{ termsLower.checklists }}.
      </p>
    </div>
...
// Get area IDs from pending locations
const pendingAreaIds = computed(() => {
  const areaIds = new Set();
  for (const locId of props.pendingLocationIds) {
    const location = props.allLocations.find(l => l.id === locId);
    if (location) {
      const locAreaIds = location.areaIds || location.AreaIds || [];
      locAreaIds.forEach(id => areaIds.add(id));
    }
  }
  return [...areaIds];
});

// Check if a scope configuration matches the pending assignments
const configMatchesPendingAssignments = (config) => {
  const scope = config.scope;
  const itemType = config.itemType;
  const ids = config.ids || [];

  // SpecificPeople with ALL_MARSHALS means everyone
  if (scope === 'SpecificPeople' && itemType === 'Marshal') {
    return ids.includes('ALL_MARSHALS');
  }

  // EveryoneAtCheckpoints or OnePerCheckpoint with Checkpoint itemType
  if ((scope === 'EveryoneAtCheckpoints' || scope === 'OnePerCheckpoint') && itemType === 'Checkpoint') {
    if (ids.includes('ALL_CHECKPOINTS')) return true;
    return ids.some(id => props.pendingLocationIds.includes(id));
  }

  // OnePerCheckpoint filtered by areas
  if (scope === 'OnePerCheckpoint' && itemType === 'Area') {
    if (ids.includes('ALL_AREAS')) return true;
    return ids.some(id => pendingAreaIds.value.includes(id));
  }
...
// Filter checklist items that would apply to this marshal
const filteredItems = computed(() => {
  return props.allChecklistItems.filter(item => {
    ...
    if (item.scopeConfigurations && item.scopeConfigurations.length > 0) {
      return item.scopeConfigurations.some(config => configMatchesPendingAssignments(config));
    }
    return false;
  });
});
...
const getScopeLabel = (item) => {
  ...
  const matchingConfig = item.scopeConfigurations.find(config => configMatchesPendingAssignments(config));
  if (!matchingConfig) {
  ...
  if (ids.includes('ALL_MARSHALS')) return 'Everyone';
...
const getScopeTooltip = (item) => {
  ...
  const tooltips = {
    'SpecificPeople': 'This item is for everyone at the event',
...
const getContextInfo = (item) => {
  ...
  if (ids.includes('ALL_MARSHALS') || ids.includes('ALL_CHECKPOINTS') || ids.includes('ALL_AREAS')) {
    return null;
  }

  // Show which checkpoints this applies to
  if (itemType === 'Checkpoint') {
    const matchingLocations = props.allLocations
      .filter(l => ids.includes(l.id) && props.pendingLocationIds.includes(l.id))
      .map(l => l.name);
    if (matchingLocations.length > 0) {
      return `At: ${matchingLocations.join(', ')}`;
    }
  }
  ...
  if (itemType === 'Area') {
    const matchingAreas = props.areas
      .filter(a => ids.includes(a.id) && pendingAreaIds.value.includes(a.id))
      .map(a => a.name);
