  ...
  <div class="notes-preview">
    ...
    <div class="preview-actions">
      <p class="preview-header">
        {{ isEditing ? `Add new note for this ${termsLower.person}:` : `Notes that will be visible to this ${termsLower.person}:` }}
      </p>
      <button
        type="button"
        class="add-item-btn"
    ...
    <div v-if="!isEditing && filteredNotes.length === 0 && pendingNewNotes.length === 0" class="empty-state">
      <p>No notes will be visible based on the current {{ termsLower.checkpoint }} assignments.</p>
      <p v-if="pendingLocationIds.length === 0" class="help-text">
        Assign {{ termsLower.checkpoints }} to see relevant notes.
      </p>
    </div>
...
const configMatchesPendingAssignments = (config) => {
  ...
  // EveryoneAtCheckpoints with Checkpoint itemType
...
// Filter notes that would be visible to this marshal
const filteredNotes = computed(() => {
  return props.allNotes.filter(note => {
    ...
    if (note.scopeConfigurations && note.scopeConfigurations.length > 0) {
      return note.scopeConfigurations.some(config => configMatchesPendingAssignments(config));
    }
    return false;
  });
});
...
const getScopeLabel = (note) => {
  ...
  const matchingConfig = note.scopeConfigurations.find(config => configMatchesPendingAssignments(config));
  if (!matchingConfig) {
...
const getScopeTooltip = (note) => {
  ...
  const tooltips = {
    'SpecificPeople': 'This note is for everyone at the event',
